...
class ActorEvents_41 extends ActorScript
{
	...
	override public function init()
	{
		...
		/* ======================== When Creating ========================= */
		actor.growTo(200/100, 200/100, 2.5, Easing.quadInOut);
		...
		runPeriodically(1000 * .25, function(timeTask:TimedTask):Void
		{
			if(wrapper.enabled)
			{
				...
				if(((Engine.engine.getGameAttribute("Boss health") : Float) <= 0))
				{
					recycleActor(actor);
		...
		runPeriodically(1000 * _BossAttack, function(timeTask:TimedTask):Void
		{
			if(wrapper.enabled)
			{
				if((actor.getAnimation() == "Phase 1"))
				{
					...
					getLastCreatedActor().applyImpulse(((Engine.engine.getGameAttribute("Wizzy1X") : Float) - actor.getX()), ((Engine.engine.getGameAttribute("Wizzy1Y") : Float) - actor.getY()), 15);
				}
				if((actor.getAnimation() == "Phase 2"))
				{
					createRecycledActor(getActorType(47), actor.getX(), actor.getY(), Script.FRONT);
					getLastCreatedActor().applyImpulse(((Engine.engine.getGameAttribute("Wizzy1X") : Float) - actor.getX()), ((Engine.engine.getGameAttribute("Wizzy1Y") : Float) - actor.getY()), 30);
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				g.translateToScreen();
				g.drawString("" + (Engine.engine.getGameAttribute("Boss health") : Float), actor.getX(), actor.getY());
			}
		});
